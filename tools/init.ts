...
const renameFiles = [
  ["src/library.ts", "src/mini-axios.ts"],
  ["test/library.test.ts", "test/mini-axios.test.ts"]
]
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/mini-axios/g, /--username--/g, /--usermail--/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function(files) {
    ...
    let newFilename = files[1].replace(/mini-axios/g, libraryName)
    mv(
